 ...
 *   node backend/scripts/checkSmtp.js --env .env
 *
 * NOTHING NEEDS TO BE DEPLOYED. This file is standalone — one dependency
 * (nodemailer, already in backend/package.json) and no database, config or
 * other project file. Drop it anywhere and run it. Against production
 * credentials without deploying anything:
 *
 *     railway run --service <worker> node scripts/checkSmtp.js
 *
 * `railway run` executes LOCALLY with that service's environment injected, so
 * you are testing the real credentials from your own machine.
 *
...
// `--env`, NOT `--env-file`. Node 20+ has its own native --env-file flag and
// validates it even when it appears AFTER the script name: with a missing file
// node exits 9 with "node: <file>: not found" before this script runs at all,
// which looks like the diagnostic is broken. Verified on Node v22.
const envFile  = args.includes('--env') ? args[args.indexOf('--env') + 1] : null;

// ── Optional .env loading, with no dependency ───────────────────────────────
// dotenv IS in backend/package.json, but requiring it would tie this script to
// having node_modules installed in the right place — and the whole point is
// that it runs from anywhere with nothing set up. Twelve lines of parsing beats
// a dependency for a diagnostic.
//
// Handles: comments, blank lines, `export FOO=bar`, and quoted values. It does
// NOT do variable interpolation — if your .env uses ${VAR}, load it with
// dotenv instead.
if (envFile) {
  const fs = require('fs');
  let raw;
  try {
    ...
    console.error(`\nCannot read --env ${envFile}: ${e.message}\n`);
    process.exit(1);
  }
  let loaded = 0;
  for (let line of raw.split(/\r?\n/)) {
    ...
    // `SMTP_HOST=x node checkSmtp.js --env .env` overrides as expected.
    if (process.env[key] === undefined) { process.env[key] = val; loaded += 1; }
  }
  console.log(`\nLoaded ${loaded} variable(s) from ${envFile}`);
}

// nodemailer is the script's ONLY dependency. Resolved with a clear message
// rather than a stack trace, because "cannot find module" from a diagnostic
// tool reads like the diagnostic is broken when it just needs a different cwd.
let nodemailer;
try {
  nodemailer = require('nodemailer');
} catch {
  console.error(`
Cannot find 'nodemailer'.

  It is already a dependency of the backend, so the usual fix is to run this
  from that directory:

      cd backend && node scripts/checkSmtp.js

  Or install it just for this:

      npm install nodemailer && node scripts/checkSmtp.js
`);
  process.exit(1);
}
