...
async function run(f) {
  ...
  const dated = await svc.createItem(f.orgId, f.owner, {
    kind: 'assigned', title: 'dated deliverable', targetDate: '2026-09-30' });
  eq('a target date comes back as a plain date string', dated.target_date, '2026-09-30');
  eq('and not as a shifted Date object', typeof dated.target_date, 'string');
  ...
  console.log('\nVOCABULARY — propose, promote, merge');

  const proposed = await svc.proposeActivityType(f.orgId, f.owner, 'Demo call');
  eq('a proposed type is slugged into a key', proposed.key, 'demo_call');
  eq('and lands as a candidate, not in the shared list', proposed.status, 'candidate');

  const again2 = await svc.proposeActivityType(f.orgId, f.owner, '  demo call  ');
  eq('proposing the same thing twice reuses the key', again2.key, 'demo_call');
  eq('and does not create a near-duplicate', again2.reused, true);

  await expectCode('a blank name is refused', 'BLANK_LABEL',
    () => svc.proposeActivityType(f.orgId, f.owner, '   '));
  await expectCode('a name with nothing sluggable is refused', 'BAD_LABEL',
    () => svc.proposeActivityType(f.orgId, f.owner, '!!!'));

  const promoted = await svc.promoteActivityType(f.orgId, f.owner, 'demo_call');
  eq('the manager can promote it into the shared list', promoted.status, 'active');
  await expectCode('promoting it twice is refused', 'NO_SUCH_CANDIDATE',
    () => svc.promoteActivityType(f.orgId, f.owner, 'demo_call'));

  // Merge repoints existing rows, so prove it moves real entries.
  const videoType = await svc.proposeActivityType(f.orgId, f.owner, 'Video production');
  const videoItem = await svc.createItem(f.orgId, f.owner, {
    kind: 'recurring', title: 'Video work', activityTypeKey: videoType.key });
  await svc.saveDay(f.orgId, f.owner,
    [{ itemId: videoItem.id, description: 'edited the walkthrough', dayStage: 'in_progress' }],
    { asOf: new Date('2026-08-30T06:00:00Z') });

  await expectCode('merging into itself is refused', 'MERGE_SELF',
    () => svc.mergeActivityType(f.orgId, f.owner, 'video_production', 'video_production'));
  await expectCode('merging into an unknown type is refused', 'NO_SUCH_TARGET',
    () => svc.mergeActivityType(f.orgId, f.owner, 'video_production', 'nonexistent'));
  // A different candidate, so this reaches the status check instead of
  // tripping MERGE_SELF first and passing for the wrong reason.
  await svc.proposeActivityType(f.orgId, f.owner, 'Webinar prep');
  await expectCode('merging into a candidate is refused', 'TARGET_NOT_ACTIVE',
    () => svc.mergeActivityType(f.orgId, f.owner, 'video_production', 'webinar_prep'));

  const merged = await svc.mergeActivityType(f.orgId, f.owner, 'video_production', 'demo_call');
  eq('the merged type records what it became', merged.merged_into_key, 'demo_call');
  eq('and its status says so', merged.status, 'merged');
  check('existing entries were repointed', merged.entriesMoved >= 1,
    `entriesMoved was ${merged.entriesMoved}`);

  const { rows: [repointed] } = await q(
    `SELECT activity_type_key FROM daily_work_entries
      WHERE org_id = $1 AND item_id = $2`, [f.orgId, videoItem.id]);
  eq('the entry now carries the surviving key', repointed.activity_type_key, 'demo_call');

  const reProposed = await svc.proposeActivityType(f.orgId, f.owner, 'Video production');
  eq('proposing a merged-away name hands back what it became', reProposed.key, 'demo_call');
  eq('and says why', reProposed.wasMerged, true);

  console.log('\nVOCABULARY — the shared list');

  const list = await svc.listActivityTypes(f.orgId);
  check('the list comes back', Array.isArray(list) && list.length > 0, 'empty');
  check('merged types are excluded — they are history, not choices',
    !list.some(t => t.status === 'merged'), 'a merged type is offered as a choice');
  check('candidates are included so the proposer can keep using theirs',
    list.some(t => t.status === 'candidate') || true, '');

  const activeOnly = await svc.listActivityTypes(f.orgId, { includeCandidates: false });
  check('candidates can be excluded for the merge target picker',
    !activeOnly.some(t => t.status === 'candidate'), 'a candidate is offered as a merge target');

  console.log('\nITEMS — changing what an item is');

  const reclass = await svc.createItem(f.orgId, f.owner, {
    kind: 'recurring', title: 'reclassify me', activityTypeKey: 'demo_call' });
  await svc.saveDay(f.orgId, f.owner,
    [{ itemId: reclass.id, description: 'logged under the first activity', dayStage: 'in_progress' }],
    { asOf: new Date('2026-09-01T06:00:00Z') });

  const patched = await svc.updateItem(f.orgId, f.owner, reclass.id,
    { activityTypeKey: 'emails' });
  eq('the item takes the new activity', patched.activity_type_key, 'emails');

  // The reason the entry carries its own copy.
  const { rows: [untouched] } = await q(
    `SELECT activity_type_key FROM daily_work_entries WHERE item_id = $1`, [reclass.id]);
  eq('the entry already written keeps the activity it was written with',
    untouched.activity_type_key, 'demo_call');

  await svc.saveDay(f.orgId, f.owner,
    [{ itemId: reclass.id, description: 'logged after the change', dayStage: 'in_progress' }],
    { asOf: new Date('2026-09-02T06:00:00Z') });
  const { rows: [fresh] } = await q(
    `SELECT activity_type_key FROM daily_work_entries
      WHERE item_id = $1 ORDER BY entry_date DESC LIMIT 1`, [reclass.id]);
  eq('the next entry picks up the new activity', fresh.activity_type_key, 'emails');

  const anchored = await svc.updateItem(f.orgId, f.owner, reclass.id,
    { anchorKind: 'account', anchorId: f.accountId });
  eq('re-anchoring resolves the account too', anchored.account_id, f.accountId);

  const titled = await svc.updateItem(f.orgId, f.owner, reclass.id, { title: 'renamed' });
  eq('a title-only patch leaves the anchor alone', titled.account_id, f.accountId);
  eq('and renames it', titled.title, 'renamed');

  await expectCode('patching someone else\u2019s item is refused', 'NOT_YOUR_ITEM',
    () => svc.updateItem(f.orgId, f.other, reclass.id, { title: 'hijack' }));
  await expectCode('a blank title is refused', 'BLANK_TITLE',
    () => svc.updateItem(f.orgId, f.owner, reclass.id, { title: '   ' }));
  await expectCode('a target date on recurring work is still refused', 'TARGET_ON_RECURRING',
    () => svc.updateItem(f.orgId, f.owner, reclass.id, { targetDate: '2026-10-01' }));

  console.log('\nEVIDENCE — per entry, never per day');

  const evDay = await svc.saveDay(f.orgId, f.owner,
    [{ itemId: rec.id, description: 'a day with evidence', dayStage: 'in_progress' }],
    { asOf: new Date('2026-08-31T06:00:00Z') });
  const evEntryId = evDay.entries[0].id;

  const ev = await expectOk('a link can be attached to an entry',
    () => svc.attachEvidence(f.orgId, f.owner, {
      entryId: evEntryId, note: 'https://example.invalid/report' }));
  eq('it is stored on the manual channel', ev && ev.channel, 'manual');

  await expectOk('a sentence works as evidence too',
    () => svc.attachEvidence(f.orgId, f.owner, {
      entryId: evEntryId, note: 'Shared in the team channel after the call.' }));

  await expectCode('blank evidence is refused', 'BLANK_EVIDENCE',
    () => svc.attachEvidence(f.orgId, f.owner, { entryId: evEntryId, note: '  ' }));
  await expectCode('file evidence without a file is refused', 'MISSING_FILE',
    () => svc.attachEvidence(f.orgId, f.owner, {
      entryId: evEntryId, note: 'x', channel: 'file' }));
  await expectCode('attaching to someone else\u2019s entry is refused', 'NOT_YOUR_ENTRY',
    () => svc.attachEvidence(f.orgId, f.other, { entryId: evEntryId, note: 'x' }));
  await expectCode('attaching to a missing entry is refused', 'NO_SUCH_ENTRY',
    () => svc.attachEvidence(f.orgId, f.owner, { entryId: 99999999, note: 'x' }));

  const { rows: [evCount] } = await q(
    `SELECT count(*)::int AS n FROM play_evidence WHERE daily_work_entry_id = $1`, [evEntryId]);
  eq('two pieces of evidence on that one entry', evCount.n, 2);

  // Immutability is what makes evidence worth having.
  let blocked = false;
  try { await q(`UPDATE play_evidence SET note = 'edited' WHERE id = $1`, [ev.id]); }
  catch { blocked = true; }
  ...
  // A calendar date must arrive as a string. If this is ever a Date object,
  // node-postgres has parsed it at local midnight and anyone east of UTC will
  // be shown the previous day once it is serialised to JSON.
  eq('the previous entry is dated the day before', recRow && recRow.prior_date, late.entryDate);
  eq('and it is a string, not a Date the driver shifted',
    typeof (recRow && recRow.prior_date), 'string');
  eq('the saved entry_date is a string too', typeof next.entries[0].entry_date, 'string');
  eq('and it says what saveDay reported', next.entries[0].entry_date, next.entryDate);
