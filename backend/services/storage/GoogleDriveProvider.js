 ...
 *
 * webViewLink: Google Drive's equivalent of OneDrive's webUrl.
 * Opens the file in Google Docs/Sheets/Drive in the browser — no download.
 */
...
class GoogleDriveProvider extends StorageProviderBase {
  ...
  async listFiles(userId, folderId = null) {
    ...
    const response = await axios.get(`${DRIVE_BASE}/files`, {
      ...
      params: {
        ...
        // webViewLink included so the list view can surface the open link immediately
  ...
  async extractFileContent(userId, fileId) {
    ...
    const effectiveMimeType = isGoogleNative
      ? GOOGLE_NATIVE_EXPORT_MAP[meta.mimeType]
      : meta.mimeType;
    ...
    return {
      ...
      // fileRef is persisted to storage_files — the file itself stays in Google Drive
      fileRef: {
        provider:          this.providerId,
        provider_file_id:  meta.id,
        web_url:           meta.webViewLink,  // opens file in Google Docs/Drive in browser
        file_name:         meta.name,
        file_size:         meta.size || buffer.length,
        mime_type:         meta.mimeType,     // original Google mime type for display
        category:          meta.category,
      ...
      metadata: {
        ...
        wasExported: isGoogleNative,
        originalMimeType: isGoogleNative ? meta.mimeType : undefined,
      },
    };
  }
  ...
  async _getAccessToken(userId) {
    ...
    if (now < expiresAt) return tokenData.access_token;

    if (!tokenData.refresh_token) {
      throw new Error(
        'Google Drive token expired and no refresh token available. Please reconnect Google Drive.'
      );
    }
  ...
  _normalize(item) {
    ...
    return {
      ...
      mimeType: isFolder ? null : item.mimeType,
      isFolder,
      childCount: 0,
      parentFolder: null,
      webViewLink: item.webViewLink || null,  // direct link to open in browser
