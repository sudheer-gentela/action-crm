 ...
 *
 * OAUTH tokens — Google and Microsoft access/refresh credentials in the
 * oauth_tokens table. Nothing to do with AI usage.
 *
 * Renamed from tokenService.js. The old name was the only file in this
 * directory without a purpose header, and "token" means two unrelated things
 * here:
 *
 *   oauthTokenService.js      → OAuth credentials            (this file)
 *   TokenTrackingService.js   → AI token usage per user/org
 *   SenderTokenHealth.js      → OAuth health for prospecting senders
 *
 * Exports: getTokenByUserId, saveUserToken, refreshUserToken, deleteUserTokens.
 * Microsoft refresh delegates to config/microsoftScopes.refreshMicrosoftToken so
 * the scope list lives in exactly one place.
 */
...
async function refreshUserToken(userId, provider) {
  ...
  try {
    ...
    if (provider === 'google') {
      ...
      // ── Microsoft refresh ──
      // This used to hold its own hard-coded copy of the scope list, which is
      // how a scope change could be applied in outlookService and silently
      // undone on the next refresh. One list now, in config/microsoftScopes.js.
      const { refreshMicrosoftToken } = require('../config/microsoftScopes');
      const { data: refreshed, downgraded } = await refreshMicrosoftToken(axios, currentToken.refresh_token);
      if (downgraded) {
        console.warn(
          `⚠️  User ${userId} has not consented to Files.ReadWrite — refreshed on their ` +
          `existing scopes. Mail and calendar are unaffected; writing files to OneDrive ` +
          `will fail until they reconnect their Microsoft account.`
        );
      }
      data = refreshed;
    }
