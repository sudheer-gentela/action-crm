...
- `pitch` — optional string. A short campaign-level narrative of what we say to this audience and why. When non-empty, treat it as the FRAMING behind the artifact's angle — paraphrased, never quoted, and never at the cost of the character caps (connection requests usually have no room for pitch content at all; it matters most for `post_accept_message` and `nurture_dm`). When empty/absent, work from `products` + `value_props` as before.
