...
--
-- Name: reschedule_go_live_anchored_plays(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.reschedule_go_live_anchored_plays() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  delta_days integer;
...
--
-- Name: touch_sales_handover_commitments(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.touch_sales_handover_commitments() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  NEW.updated_at := now();
...
--
-- Name: touch_whatsapp_thread_window(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.touch_whatsapp_thread_window() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE TABLE public.sales_handover_commitments (
    ...
    closure_note text,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT sales_handover_commitments_closed_at_check CHECK (((((status)::text = ANY ((ARRAY['met'::character varying, 'waived'::character varying, 'breached'::character varying])::text[])) AND (closed_at IS NOT NULL)) OR (((status)::text = ANY ((ARRAY['open'::character varying, 'in_progress'::character varying])::text[])) AND (closed_at IS NULL)))),
    CONSTRAINT sales_handover_commitments_closure_note_check CHECK ((((status)::text <> ALL ((ARRAY['waived'::character varying, 'breached'::character varying])::text[])) OR ((closure_note IS NOT NULL) AND (length(btrim(closure_note)) > 0)))),
    ...
    CONSTRAINT sales_handover_commitments_status_check CHECK (((status)::text = ANY ((ARRAY['open'::character varying, 'in_progress'::character varying, 'met'::character varying, 'waived'::character varying, 'breached'::character varying])::text[])))
);
...
--
-- Name: COLUMN sales_handover_commitments.due_date; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sales_handover_commitments.due_date IS 'Date the commitment was promised for. Drives HandoverRulesEngine.handover_commitment_overdue. NULL = no date agreed (allowed; the rule simply skips it).';


--
-- Name: COLUMN sales_handover_commitments.owner_user_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sales_handover_commitments.owner_user_id IS 'Who owes this. Usually the assigned_service_owner post-acknowledgement, but a commitment can be owned by the sales rep (e.g. a pricing concession they must formalise).';


--
-- Name: COLUMN sales_handover_commitments.status; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN sales_handovers.closure_summary; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: handover_deliverable_rollup; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.handover_deliverable_rollup AS
...
--
-- Name: org_whatsapp_accounts; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.org_whatsapp_accounts (
    org_id integer NOT NULL,
    ...
    is_official_business_account boolean DEFAULT false NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT org_whatsapp_accounts_provider_chk CHECK ((provider = ANY (ARRAY['meta_cloud'::text, 'twilio'::text]))),
    CONSTRAINT org_whatsapp_accounts_status_chk CHECK ((status = ANY (ARRAY['active'::text, 'suspended'::text, 'revoked'::text])))
);


--
-- Name: TABLE org_whatsapp_accounts; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.org_whatsapp_accounts IS 'Per-org WhatsApp Business Account. provider=meta_cloud talks to Graph directly; provider=twilio routes through the org''s existing Twilio subaccount (org_twilio_accounts). Groups API requires meta_cloud + is_official_business_account.';


--
-- Name: COLUMN org_whatsapp_accounts.is_official_business_account; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.org_whatsapp_accounts.is_official_business_account IS 'Meta OBA status. Gates the Groups API ΓÇö without it, group creation returns a permissions error regardless of scopes.';
...
--
-- Name: COLUMN playbook_plays.due_anchor; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.playbook_plays.due_anchor IS 'What due_offset_days is measured from. created = instantiation date (default, preserves existing behaviour for every sales play). go_live = sales_handovers.go_live_date, where due_offset_days is normally NEGATIVE (e.g. -14 for "UAT sign-off two weeks before go-live").';
...
CREATE TABLE public.sequence_enrollments (
    ...
    pinned_sender_account_id integer,
    thread_conversation_id text,
    thread_root_subject text,
...
--
-- Name: COLUMN sequence_enrollments.pinned_sender_account_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequence_enrollments.pinned_sender_account_id IS 'The prospecting_sender_accounts.id this enrollment is pinned to once its first email sends (threading or pin_sender). NULL until the first email; after that the firer restricts the sender pool to this id. Soft reference ΓÇö app logic owns integrity.';


--
-- Name: COLUMN sequence_enrollments.thread_conversation_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequence_enrollments.thread_conversation_id IS 'Immutable thread anchor stamped at the first email: Gmail threadId / Outlook conversationId. Passed to the provider on every subsequent email step so it nests in the same server-side thread.';


--
-- Name: COLUMN sequence_enrollments.thread_root_subject; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequence_enrollments.thread_root_subject IS 'Subject of the enrollment''s first email. Reused (optionally Re:-prefixed per sequences.thread_subject_mode) on all later email steps so the thread stays coherent.';


--
-- Name: COLUMN sequence_enrollments.thread_last_message_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequence_enrollments.thread_last_message_id IS 'RFC822 Message-ID of the most recent email sent for this enrollment. Set as In-Reply-To on the next email step.';


--
-- Name: COLUMN sequence_enrollments.thread_references; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequence_enrollments.thread_references IS 'Accumulated RFC822 References chain (space-separated Message-IDs, root first). Grows one id per email step; sent as the References header so non-provider recipient clients also thread correctly.';
...
CREATE TABLE public.sequences (
    ...
    thread_replies boolean DEFAULT false NOT NULL,
...
--
-- Name: COLUMN sequences.thread_replies; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequences.thread_replies IS 'When true, email steps after the first are sent as replies into the enrollment''s first-email thread (subject reused, In-Reply-To/References carried). Forces sender pinning on. Opt-in per sequence; default false = today''s standalone sends.';


--
-- Name: COLUMN sequences.pin_sender; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequences.pin_sender IS 'Standalone "pin to one mailbox, stop rotation" toggle. Independent of threading, but threading forces pinning on regardless of this flag. v1 pins to whichever sender sends the enrollment''s first email; a named-mailbox selection can layer on later.';


--
-- Name: COLUMN sequences.thread_subject_mode; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.sequences.thread_subject_mode IS 'keep = reply uses the root subject verbatim; re = root subject prefixed with "Re: " (only if not already present). Applies to steps 2+ in threaded mode.';


--
-- Name: COLUMN sequences.thread_failover_mode; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN whatsapp_messages.capture_meta; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_messages.capture_meta IS 'Session-capture provenance, mirroring the LinkedIn extension''s capture_meta discipline: { sessionId, workerVersion, baileysVersion, jid, participantJid, receivedAt, msgType }. Lets us tell later which worker build produced a row when a normaliser bug is found.';
...
--
-- Name: whatsapp_session_auth; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE whatsapp_session_auth; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_session_groups; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.whatsapp_session_groups (
    id integer NOT NULL,
    session_id integer NOT NULL,
    org_id integer NOT NULL,
...
--
-- Name: COLUMN whatsapp_session_groups.is_watched; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_session_groups_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.whatsapp_session_groups_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
--
-- Name: whatsapp_session_groups_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.whatsapp_session_groups_id_seq OWNED BY public.whatsapp_session_groups.id;
...
CREATE TABLE public.whatsapp_sessions (
    ...
    capture_media boolean DEFAULT false NOT NULL,
    created_by integer,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    ...
    batch_max integer DEFAULT 50 NOT NULL,
    stale_socket_minutes integer DEFAULT 20 NOT NULL,
    reconnect_max_seconds integer DEFAULT 300 NOT NULL,
...
COMMENT ON TABLE public.whatsapp_sessions IS 'A WhatsApp number observed via a companion-device client. Read-only by design: this table has no send-side columns and the worker never calls sendMessage or readMessages.';


--
-- Name: COLUMN whatsapp_sessions.phone_last_seen_at; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_sessions.phone_last_seen_at IS 'When the PRIMARY handset was last confirmed opened. Not detectable from the protocol ΓÇö set by a human via the admin UI. WhatsApp unlinks all companion devices at 14 days.';


--
-- Name: COLUMN whatsapp_sessions.heartbeat_at; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_sessions.heartbeat_at IS 'Written on a timer whether or not messages arrive. Staleness here means the worker is not running; staleness in last_seen_at only means nobody messaged.';


--
-- Name: COLUMN whatsapp_sessions.stale_socket_minutes; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_sessions.stale_socket_minutes IS 'Watchdog threshold. Baileys can hold a socket WhatsApp has abandoned ΓÇö connection.update never fires, messages never arrive, and status stays "connected". This is what catches that.';


--
-- Name: COLUMN whatsapp_sessions.capture_mode; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_session_health; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.whatsapp_session_health AS
 SELECT id,
    org_id,
    wa_phone,
    status,
    heartbeat_at,
    (EXTRACT(epoch FROM (now() - heartbeat_at)) / (60)::numeric) AS heartbeat_stale_minutes,
    (EXTRACT(epoch FROM (now() - phone_last_seen_at)) / (86400)::numeric) AS phone_stale_days,
    reconnect_count,
    last_reconnect_at,
    ((status = 'connected'::text) AND (heartbeat_at IS NOT NULL) AND (heartbeat_at < (now() - (((heartbeat_seconds * 3) || ' seconds'::text))::interval))) AS heartbeat_stale,
    (status = 'logged_out'::text) AS needs_rescan
...
--
-- Name: COLUMN whatsapp_templates.category; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_templates.category IS 'Drives cost. In India (Jan 2026 rate card) UTILITY is ~Rs 0.115 per delivered message vs MARKETING at ~Rs 0.863 ΓÇö roughly 7.5x. Every operational GoWarmCRM notification (milestone due, sign-off request, status change) is legitimately UTILITY. Mis-categorising these as MARKETING is the single most expensive mistake available on this channel.';


--
-- Name: COLUMN whatsapp_templates.variable_map; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_templates.variable_map IS 'Ordered array of dot-paths resolved against the send context, e.g. ["stakeholder.name","commitment.description","commitment.dueDate"] maps to {{1}},{{2}},{{3}}.';
...
--
-- Name: whatsapp_thread_participants; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.whatsapp_thread_participants (
    id integer NOT NULL,
    thread_id integer NOT NULL,
    org_id integer NOT NULL,
...
--
-- Name: whatsapp_thread_participants_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.whatsapp_thread_participants_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
--
-- Name: whatsapp_thread_participants_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.whatsapp_thread_participants_id_seq OWNED BY public.whatsapp_thread_participants.id;


--
-- Name: whatsapp_threads; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.whatsapp_threads (
    ...
    CONSTRAINT whatsapp_threads_source_chk CHECK ((source = ANY (ARRAY['cloud_api'::text, 'session'::text]))),
    CONSTRAINT whatsapp_threads_status_chk CHECK ((status = ANY (ARRAY['active'::text, 'archived'::text, 'blocked'::text])))
);


--
-- Name: COLUMN whatsapp_threads.wa_group_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_threads.wa_group_id IS 'Groups API group ID. Only ever set by group creation through our own API call ΓÇö an existing consumer-app group cannot be adopted, and only one Cloud API business may occupy a group.';


--
-- Name: COLUMN whatsapp_threads.window_expires_at; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.whatsapp_threads.window_expires_at IS 'last_inbound_at + 24h. While in the future, free-form (non-template) sends are permitted and currently free. Note: Meta has announced service-message pricing changes effective 1 Oct 2026 ΓÇö the permission stays, the "free" part does not. Do not hard-code the assumption that inside-window sends cost nothing.';


--
-- Name: COLUMN whatsapp_threads.source; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_threads_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.whatsapp_threads_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
--
-- Name: whatsapp_threads_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.whatsapp_threads_id_seq OWNED BY public.whatsapp_threads.id;
...
--
-- Name: whatsapp_session_groups id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_session_groups ALTER COLUMN id SET DEFAULT nextval('public.whatsapp_session_groups_id_seq'::regclass);
...
--
-- Name: whatsapp_thread_participants id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_thread_participants ALTER COLUMN id SET DEFAULT nextval('public.whatsapp_thread_participants_id_seq'::regclass);


--
-- Name: whatsapp_threads id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_threads ALTER COLUMN id SET DEFAULT nextval('public.whatsapp_threads_id_seq'::regclass);
...
--
-- Name: org_whatsapp_accounts org_whatsapp_accounts_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_whatsapp_accounts
    ADD CONSTRAINT org_whatsapp_accounts_pkey PRIMARY KEY (org_id);
...
--
-- Name: whatsapp_session_auth whatsapp_session_auth_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_session_groups whatsapp_session_groups_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_session_groups
    ADD CONSTRAINT whatsapp_session_groups_pkey PRIMARY KEY (id);
...
--
-- Name: whatsapp_thread_participants whatsapp_thread_participants_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_thread_participants
    ADD CONSTRAINT whatsapp_thread_participants_pkey PRIMARY KEY (id);


--
-- Name: whatsapp_threads whatsapp_threads_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: idx_seq_enroll_thread_paused; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_seq_enroll_thread_paused ON public.sequence_enrollments USING btree (org_id, enrolled_by) WHERE (((status)::text = 'paused'::text) AND ((stop_reason)::text = 'thread_sender_blocked'::text));
...
--
-- Name: idx_shc_handover_open_due; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_shc_handover_open_due ON public.sales_handover_commitments USING btree (handover_id, due_date) WHERE (((status)::text = ANY ((ARRAY['open'::character varying, 'in_progress'::character varying])::text[])) AND (due_date IS NOT NULL));


--
-- Name: idx_shc_org_open_due; Type: INDEX; Schema: public; Owner: -
--
...
CREATE INDEX idx_wa_messages_org_time ON public.whatsapp_messages USING btree (org_id, created_at DESC);
...
--
-- Name: idx_wa_session_groups_triage; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wa_session_groups_triage ON public.whatsapp_session_groups USING btree (org_id, binding_status, last_message_at DESC);


--
-- Name: idx_wa_session_groups_watched; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wa_session_groups_watched ON public.whatsapp_session_groups USING btree (session_id, group_jid) INCLUDE (is_watched, binding_status);
...
--
-- Name: idx_wa_templates_purpose; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wa_templates_purpose ON public.whatsapp_templates USING btree (org_id, purpose) WHERE (status = 'approved'::text);


--
-- Name: idx_wa_threads_handover; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wa_threads_handover ON public.whatsapp_threads USING btree (handover_id) WHERE (handover_id IS NOT NULL);


--
-- Name: idx_wa_threads_session_unbound; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wa_threads_session_unbound ON public.whatsapp_threads USING btree (org_id, updated_at DESC) WHERE ((source = 'session'::text) AND (handover_id IS NULL));
...
--
-- Name: uq_wa_messages_wamid; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_wa_messages_wamid ON public.whatsapp_messages USING btree (org_id, wa_message_id) WHERE (wa_message_id IS NOT NULL);


--
-- Name: uq_wa_participants; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_wa_participants ON public.whatsapp_thread_participants USING btree (thread_id, wa_phone);


--
-- Name: uq_wa_session_groups_jid; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_wa_session_groups_jid ON public.whatsapp_session_groups USING btree (session_id, group_jid);


--
-- Name: uq_wa_sessions_org; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_wa_sessions_org ON public.whatsapp_sessions USING btree (org_id) WHERE (status <> 'disabled'::text);


--
-- Name: uq_wa_templates_org_name_lang; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_wa_templates_org_name_lang ON public.whatsapp_templates USING btree (org_id, name, language);


--
-- Name: uq_wa_threads_direct; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_wa_threads_direct ON public.whatsapp_threads USING btree (org_id, wa_phone) WHERE (kind = 'direct'::text);


--
-- Name: uq_wa_threads_group; Type: INDEX; Schema: public; Owner: -
--
...
CREATE TRIGGER trg_reschedule_go_live AFTER UPDATE OF go_live_date ON public.sales_handovers FOR EACH ROW EXECUTE FUNCTION public.reschedule_go_live_anchored_plays();
...
--
-- Name: sales_handover_commitments trg_touch_shc; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_touch_shc BEFORE UPDATE ON public.sales_handover_commitments FOR EACH ROW EXECUTE FUNCTION public.touch_sales_handover_commitments();


--
-- Name: whatsapp_messages trg_wa_thread_window; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_wa_thread_window AFTER INSERT ON public.whatsapp_messages FOR EACH ROW EXECUTE FUNCTION public.touch_whatsapp_thread_window();
...
ALTER TABLE ONLY public.actions
    ADD CONSTRAINT actions_playbook_play_id_fkey FOREIGN KEY (playbook_play_id) REFERENCES public.playbook_plays(id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.case_plays
    ADD CONSTRAINT case_plays_play_id_fkey FOREIGN KEY (play_id) REFERENCES public.playbook_plays(id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.contract_play_instances
    ADD CONSTRAINT contract_play_instances_play_id_fkey FOREIGN KEY (play_id) REFERENCES public.playbook_plays(id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.contract_plays
    ADD CONSTRAINT contract_plays_play_id_fkey FOREIGN KEY (play_id) REFERENCES public.playbook_plays(id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY public.deal_play_instances
    ADD CONSTRAINT deal_play_instances_play_id_fkey FOREIGN KEY (play_id) REFERENCES public.playbook_plays(id) ON DELETE RESTRICT;
...
--
-- Name: org_whatsapp_accounts org_whatsapp_accounts_connected_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_whatsapp_accounts
    ADD CONSTRAINT org_whatsapp_accounts_connected_by_fkey FOREIGN KEY (connected_by) REFERENCES public.users(id);


--
-- Name: org_whatsapp_accounts org_whatsapp_accounts_org_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_whatsapp_accounts
    ADD CONSTRAINT org_whatsapp_accounts_org_id_fkey FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
...
--
-- Name: CONSTRAINT prospecting_actions_play_id_fkey ON prospecting_actions; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON CONSTRAINT prospecting_actions_play_id_fkey ON public.prospecting_actions IS 'Deliberately ON DELETE SET NULL, unlike the other playbook_plays references which are RESTRICT as of 2026_72. Per D19 prospecting is volume-asymmetric (thousands vs tens) and is not the lineage surface playbook versioning or Phase D extraction reads. RESTRICT here would let one historical prospecting action block a play permanently.';
...
ALTER TABLE ONLY public.sales_handover_commitments
    ADD CONSTRAINT sales_handover_commitments_closed_by_fkey FOREIGN KEY (closed_by) REFERENCES public.users(id);
...
--
-- Name: sales_handover_commitments sales_handover_commitments_owner_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.sales_handover_commitments
    ADD CONSTRAINT sales_handover_commitments_owner_user_id_fkey FOREIGN KEY (owner_user_id) REFERENCES public.users(id);
...
--
-- Name: sales_handovers sales_handovers_cancelled_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.sales_handovers
    ADD CONSTRAINT sales_handovers_cancelled_by_fkey FOREIGN KEY (cancelled_by) REFERENCES public.users(id);


--
-- Name: sales_handovers sales_handovers_completed_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.sales_handovers
    ADD CONSTRAINT sales_handovers_completed_by_fkey FOREIGN KEY (completed_by) REFERENCES public.users(id);
...
--
-- Name: whatsapp_messages whatsapp_messages_thread_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_session_auth whatsapp_session_auth_session_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_session_groups whatsapp_session_groups_bound_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_session_groups whatsapp_session_groups_org_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_session_groups
    ADD CONSTRAINT whatsapp_session_groups_org_id_fkey FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON DELETE CASCADE;


--
-- Name: whatsapp_session_groups whatsapp_session_groups_session_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_session_groups
    ADD CONSTRAINT whatsapp_session_groups_session_id_fkey FOREIGN KEY (session_id) REFERENCES public.whatsapp_sessions(id) ON DELETE CASCADE;


--
-- Name: whatsapp_session_groups whatsapp_session_groups_thread_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_session_groups
    ADD CONSTRAINT whatsapp_session_groups_thread_id_fkey FOREIGN KEY (thread_id) REFERENCES public.whatsapp_threads(id) ON DELETE SET NULL;


--
-- Name: whatsapp_session_groups whatsapp_session_groups_watched_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_session_groups
    ADD CONSTRAINT whatsapp_session_groups_watched_by_fkey FOREIGN KEY (watched_by) REFERENCES public.users(id);


--
-- Name: whatsapp_sessions whatsapp_sessions_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_sessions
    ADD CONSTRAINT whatsapp_sessions_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id);


--
-- Name: whatsapp_sessions whatsapp_sessions_org_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_sessions
    ADD CONSTRAINT whatsapp_sessions_org_id_fkey FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON DELETE CASCADE;


--
-- Name: whatsapp_sessions whatsapp_sessions_phone_confirmed_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_sessions
    ADD CONSTRAINT whatsapp_sessions_phone_confirmed_by_fkey FOREIGN KEY (phone_confirmed_by) REFERENCES public.users(id);
...
ALTER TABLE ONLY public.whatsapp_templates
    ADD CONSTRAINT whatsapp_templates_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id);


--
-- Name: whatsapp_templates whatsapp_templates_org_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: whatsapp_thread_participants whatsapp_thread_participants_thread_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_thread_participants
    ADD CONSTRAINT whatsapp_thread_participants_thread_id_fkey FOREIGN KEY (thread_id) REFERENCES public.whatsapp_threads(id) ON DELETE CASCADE;


--
-- Name: whatsapp_thread_participants whatsapp_thread_participants_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_thread_participants
    ADD CONSTRAINT whatsapp_thread_participants_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id);


--
-- Name: whatsapp_threads whatsapp_threads_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_threads
    ADD CONSTRAINT whatsapp_threads_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id);


--
-- Name: whatsapp_threads whatsapp_threads_handover_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_threads
    ADD CONSTRAINT whatsapp_threads_handover_id_fkey FOREIGN KEY (handover_id) REFERENCES public.sales_handovers(id) ON DELETE SET NULL;


--
-- Name: whatsapp_threads whatsapp_threads_org_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.whatsapp_threads
    ADD CONSTRAINT whatsapp_threads_org_id_fkey FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
...
--
-- Name: whatsapp_session_groups whatsapp_session_groups_org_isolation; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY whatsapp_session_groups_org_isolation ON public.whatsapp_session_groups USING ((org_id = (current_setting('app.current_org_id'::text, true))::integer));
...
--
-- Name: whatsapp_sessions whatsapp_sessions_org_isolation; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY whatsapp_sessions_org_isolation ON public.whatsapp_sessions USING ((org_id = (current_setting('app.current_org_id'::text, true))::integer));
